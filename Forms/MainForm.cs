...
namespace TLO.local
{
  public class MainForm : Form
  {
    ...
    private DataGridView _dgvReportDownloads;
    ...
    private DataGridViewCheckBoxColumn ColumnReport1DgvIsKeeper;
    ...
    public MainForm()
    {
      ...
      this.Text = this.headText = string.Format("TLO {0}", (object) FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).FileVersion);
      ...
      this._dgvReportDownloads.AutoGenerateColumns = false;
      this._dgvReportDownloads.ClearSelection();
      this._dgvReportDownloads.DataSource = (object) this._TopicsSource;
      ...
      try
      {
        if (sender == this.menuSettingsToolStripMenuItem)
        {
          if (new SettingsForm().ShowDialog() == DialogResult.OK)
          {
            ...
            if (MessageBox.Show("Запустить загрузку/обновление информации о топиках (раздачах) по всем категориям?", "Обновление данных", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.OK)
          ...
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwUpdateCountSeedersByAllCategories), "Обновление кол-ва сидов на раздачах...", sender);
        else if (sender == this.UpdateListTopicsToolStripMenuItem)
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwUpdateTopicsByCategories), "Полное обновление информации о топиках (раздачах) по всем категориям...", (object) ClientLocalDB.Current.GetCategoriesEnable());
        else if (sender == this.UpdateKeepTopicsToolStripMenuItem)
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwUpdateHashFromAllTorrentClients), "Полное обновление информации из Torrent-клиентов...", (object) null);
        else if (sender == this.ClearDatabaseToolStripMenuItem)
        {
          if (MessageBox.Show("Вы пытаетесь очистить базу данны от текущих данных (статистику и информацию о топиках).\r\n Продолжить?", "Запрос подтверждения", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button2) != DialogResult.Yes)
            return;
        ...
        else if (sender == this.SendReportsToForumToolStripMenuItem)
        {
          if (MessageBox.Show("Отправка отчетов на форум может продолжаться продолжительное время.\r\n Продолжить?", "Запрос подтверждения", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button2) != DialogResult.Yes)
            return;
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwSendReports), "Отправка отчетов на форум...", (object) this);
        }
        else if (sender == this.CreateReportsToolStripMenuItem)
        {
          if (MessageBox.Show("Сборка отчетов может продолжаться продолжительное время и потребуется обновить список раздач и информацию из торрент-клиентов.\r\n Продолжит?", "Запрос подтверждения", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button2) != DialogResult.Yes)
            return;
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwUpdateCountSeedersByAllCategories), "Обновление кол-ва сидов на раздачах...", sender);
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwUpdateHashFromAllTorrentClients), "Обновление информации из Torrent-клиентов...", sender);
        }
        else if (sender == this.RuningStopingDistributionToolStripMenuItem)
        {
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwUpdateCountSeedersByAllCategories), "Обновление кол-ва сидов на раздачах...", sender);
          this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwRuningAndStopingDistributions), "Обновление информации из Torrent-клиентов...", sender);
    ...
    private void tmr_Tick(object sender, EventArgs e)
    {
      if (this.backgroundWorkers.Count > 0)
      {
        ...
        this.notifyIcon.Text = string.Format("{0} ({1})", (object) this.headText, (object) "Выполняются задачи...");
      }
      else
      {
        ...
        if (timeSpan.TotalSeconds > 0.0)
        {
          ...
          this.notifyIcon.Text = string.Format("{0} ({1:hh\\:mm\\:ss})", (object) this.headText, (object) timeSpan);
        }
        else
        {
          try
          {
            ...
            if (lastUpdateTopics < dateTime2)
            {
              ...
              this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwUpdateCountSeedersByAllCategories), "Обновление информации о кол-ве сидов на раздачах...", sender);
            this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwRuningAndStopingDistributions), "Запуск/Остановка раздач в Torrent-клиентах...", sender);
            this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwCreateReportsTorrentClients), "Построение сводного отчета по торрент-клиентам...", sender);
          }
          catch (Exception ex)
          {
    ...
    {
      if (sender == this._CategorySource || sender == this._cbCountSeeders || (sender == this._cbBlackList || sender == this._cbCategoryFilters) || sender == this._DateRegistration)
      {
        ...
        if (this._CategorySource.Current != null)
        {
          ...
          List<TopicInfo> source = !Settings.Current.IsAvgCountSeeders ? ClientLocalDB.Current.GetTopics(regTime, current.CategoryID, num > -1 ? new int?(num) : new int?(), new int?(), isKeep, isKeepers, isDownload, isBlack, isPoster) : ClientLocalDB.Current.GetTopics(regTime, current.CategoryID, new int?(), num > -1 ? new int?(num) : new int?(), isKeep, isKeepers, isDownload, isBlack, isPoster);
          this._lbTotal.Text = string.Format("Кол-во: {0}; Размер: {1}", (object) source.Count<TopicInfo>(), (object) TopicInfo.sizeToString(source.Sum<TopicInfo>((Func<TopicInfo, long>) (x => x.Size))));
      ...
      Dictionary<Tuple<int, int>, Tuple<string, string>> reports = ClientLocalDB.Current.GetReports(new int?((this._CategorySource.Current as Category).CategoryID));
      if (reports.Count<KeyValuePair<Tuple<int, int>, Tuple<string, string>>>() > 0)
      {
        ...
        foreach (KeyValuePair<Tuple<int, int>, Tuple<string, string>> keyValuePair in (IEnumerable<KeyValuePair<Tuple<int, int>, Tuple<string, string>>>) reports.OrderBy<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, int>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, int>) (x => x.Key.Item2)))
        {
    ...
    private void LinkClick(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        {
          List<TopicInfo> list = (this._TopicsSource.DataSource as List<TopicInfo>).Where<TopicInfo>((Func<TopicInfo, bool>) (x => x.Checked)).ToList<TopicInfo>();
        ...
        else if (sender == this.linkSetNewLabel)
        {
          ...
          if (getLableName.ShowDialog() == DialogResult.OK)
            this.dwCreateAndRun(new DoWorkEventHandler(Logic.bwSetLabels), "Установка пользовательских меток...", (object) new Tuple<MainForm, List<TopicInfo>, string>(this, (this._TopicsSource.DataSource as List<TopicInfo>).Where<TopicInfo>((Func<TopicInfo, bool>) (x => x.Checked)).ToList<TopicInfo>(), getLableName.Value));
        }
        this.SelectionChanged((object) this._CategorySource, (EventArgs) null);
      }
      catch (Exception ex)
    ...
    {
      if (this._dgvReportDownloads.Columns[e.ColumnIndex].DataPropertyName == "Name")
      {
        try
        {
      ...
      {
        if (!(this._dgvReportDownloads.Columns[e.ColumnIndex].DataPropertyName == "Alternative"))
          return;
        try
        {
          int? topicId = this._dgvReportDownloads.Rows[e.RowIndex].Cells[0].Value as int?;
          if (!topicId.HasValue)
            return;
          List<TopicInfo> dataSource = this._TopicsSource.DataSource as List<TopicInfo>;
          if (dataSource == null)
            return;
          TopicInfo topicInfo = dataSource.Where<TopicInfo>((Func<TopicInfo, bool>) (x => x.TopicID == topicId.Value)).FirstOrDefault<TopicInfo>();
          if (topicInfo == null)
            return;
    ...
    private void backgroundWorker1_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
    {
      ...
      if (sender != null && sender is BackgroundWorker && this.backgroundWorkers.ContainsKey(sender as BackgroundWorker))
      {
    ...
    private void backgroundWorker1_ProgressChanged(object sender, ProgressChangedEventArgs e)
    {
      ...
      this.toolStripProgressBar1.Value = e.ProgressPercentage < 0 || e.ProgressPercentage > 100 ? 100 : e.ProgressPercentage;
    }
    ...
    {
      DataGridViewColumn column = this._dgvReportDownloads.Columns[e.ColumnIndex];
      ...
      {
        DataGridViewColumn sortedColumn = this._dgvReportDownloads.SortedColumn;
        SortOrder sortOrder = column.HeaderCell.SortGlyphDirection == SortOrder.None || column.HeaderCell.SortGlyphDirection == SortOrder.Descending ? SortOrder.Ascending : SortOrder.Descending;
        if (column == null)
        {
          int num = (int) MessageBox.Show("Select a single column and try again.", "Error: Invalid Selection", MessageBoxButtons.OK, MessageBoxIcon.Hand);
        }
        else
        {
          List<TopicInfo> dataSource = this._TopicsSource.DataSource as List<TopicInfo>;
          ...
          if (column == this.ColumnReport1DgvSize)
            list2 = (sortOrder == SortOrder.Ascending ? (IEnumerable<TopicInfo>) list1.OrderBy<TopicInfo, long>((Func<TopicInfo, long>) (d => d.Size)) : (IEnumerable<TopicInfo>) list1.OrderByDescending<TopicInfo, long>((Func<TopicInfo, long>) (d => d.Size))).ToList<TopicInfo>();
          else if (column == this.ColumnReport1DgvName)
            list2 = (sortOrder == SortOrder.Ascending ? (IEnumerable<TopicInfo>) list1.OrderBy<TopicInfo, string>((Func<TopicInfo, string>) (d => d.Name)) : (IEnumerable<TopicInfo>) list1.OrderByDescending<TopicInfo, string>((Func<TopicInfo, string>) (d => d.Name))).ToList<TopicInfo>();
          else if (column == this.ColumnReport1DgvSeeders)
            list2 = (sortOrder == SortOrder.Ascending ? (IEnumerable<TopicInfo>) list1.OrderBy<TopicInfo, int>((Func<TopicInfo, int>) (d => d.Seeders)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name)) : (IEnumerable<TopicInfo>) list1.OrderByDescending<TopicInfo, int>((Func<TopicInfo, int>) (d => d.Seeders)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name))).ToList<TopicInfo>();
          else if (column == this.ColumnReport1DgvAvgSeeders)
            list2 = (sortOrder == SortOrder.Ascending ? (IEnumerable<TopicInfo>) list1.OrderBy<TopicInfo, Decimal?>((Func<TopicInfo, Decimal?>) (d => d.AvgSeeders)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name)) : (IEnumerable<TopicInfo>) list1.OrderByDescending<TopicInfo, Decimal?>((Func<TopicInfo, Decimal?>) (d => d.AvgSeeders)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name))).ToList<TopicInfo>();
          else if (column == this.ColumnReport1DgvRegTime)
          {
            list2 = (sortOrder == SortOrder.Ascending ? (IEnumerable<TopicInfo>) list1.OrderBy<TopicInfo, DateTime>((Func<TopicInfo, DateTime>) (d => d.RegTime)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name)) : (IEnumerable<TopicInfo>) list1.OrderByDescending<TopicInfo, DateTime>((Func<TopicInfo, DateTime>) (d => d.RegTime)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name))).ToList<TopicInfo>();
          }
          else
          {
            if (column != this.ColumnReport1DgvStatus)
              return;
            list2 = (sortOrder == SortOrder.Ascending ? (IEnumerable<TopicInfo>) list1.OrderBy<TopicInfo, string>((Func<TopicInfo, string>) (d => d.StatusToString)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name)) : (IEnumerable<TopicInfo>) list1.OrderByDescending<TopicInfo, string>((Func<TopicInfo, string>) (d => d.StatusToString)).ThenBy<TopicInfo, string>((Func<TopicInfo, string>) (d => this.Name))).ToList<TopicInfo>();
          }
          this._TopicsSource.Clear();
    ...
    {
      if (this._dgvReportDownloads.Columns.GetColumnCount(DataGridViewElementStates.Selected) == 1)
      {
        DataGridViewColumn selectedColumn = this._dgvReportDownloads.SelectedColumns[0];
      }
      Console.WriteLine("");
    }
    ...
    private void WriteReports()
    {
      ...
      string str1 = reports.Where<KeyValuePair<Tuple<int, int>, Tuple<string, string>>>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, bool>) (x => x.Key.Item2 == 0)).Select<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, string>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, string>) (x => x.Value.Item2)).FirstOrDefault<string>();
      this._txtConsolidatedReport.Text = string.IsNullOrWhiteSpace(str1) ? string.Empty : str1;
      string str2 = reports.Where<KeyValuePair<Tuple<int, int>, Tuple<string, string>>>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, bool>) (x => x.Key.Item2 == 1)).Select<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, string>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, string>) (x => x.Value.Item2)).FirstOrDefault<string>();
      this._tbConsolidatedTorrentClientsReport.Text = string.IsNullOrWhiteSpace(str2) ? string.Empty : str2;
      IEnumerable<Category> categories = ClientLocalDB.Current.GetCategories().Where<Category>((Func<Category, bool>) (x => x.CategoryID > 100000));
      Size size = this._tcCetegoriesRootReports.Size;
      foreach (Category category in categories)
      {
        string str3 = ClientLocalDB.Current.GetReports(new int?(category.CategoryID)).Where<KeyValuePair<Tuple<int, int>, Tuple<string, string>>>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, bool>) (x => x.Key.Item2 == 0)).Select<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, string>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, string>) (x => x.Value.Item2)).FirstOrDefault<string>();
      ...
      try
      {
        ...
        using (FileStream fileStream = File.Open(fileName, FileMode.OpenOrCreate, FileAccess.Write))
        {
          using (BinaryWriter binaryWriter = new BinaryWriter((Stream) fileStream, Encoding.UTF8))
          {
            ...
            int[] cats = ClientLocalDB.Current.GetCategoriesEnable().Select<Category, int>((Func<Category, int>) (x => x.CategoryID)).ToArray<int>();
            Dictionary<Tuple<int, int>, Tuple<string, string>> reports = ClientLocalDB.Current.GetReports(new int?());
            foreach (KeyValuePair<Tuple<int, int>, Tuple<string, string>> keyValuePair in reports.Where<KeyValuePair<Tuple<int, int>, Tuple<string, string>>>((Func<KeyValuePair<Tuple<int, int>, Tuple<string, string>>, bool>) (x => ((IEnumerable<int>) cats).Contains<int>(x.Key.Item1))))
            {
          ...
          using (BinaryReader binaryReader = new BinaryReader((Stream) fileStream))
          {
            while (binaryReader.BaseStream.Length != binaryReader.BaseStream.Position)
            {
              ...
              if (!(str == "TorrentClientInfo"))
              {
                ...
                {
                  if (str == "Report")
                    result.Add(new Tuple<int, int, string>(binaryReader.ReadInt32(), binaryReader.ReadInt32(), binaryReader.ReadString()));
                }
                else
                {
            ...
            ClientLocalDB.Current.SaveTorrentClients((IEnumerable<TorrentClientInfo>) torrentClientInfoList, false);
    ...
    private void InitializeComponent()
    {
            ...
            this.RuningStopingDistributionToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
            ...
            this.CreateConsolidatedReportByTorrentClientsToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
            ...
            this.UpdateCountSeedersToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
            ...
            this.UpdateListTopicsToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
            ...
            this.UpdateKeepTopicsToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
            ...
            this.LoadListKeepersToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
            ...
            this.ClearDatabaseToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
            ...
            this.DevlToolStripMenuItem.Size = new System.Drawing.Size(380, 22);
